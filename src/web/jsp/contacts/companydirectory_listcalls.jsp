...
<dhv:container name="contacts" selected="calls" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  ...
  <table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
    ...
    %>
    <tr class="row<%= rowid %>">
      <td <%= CallsListInfo.getExpandedSelection() && !"".equals(thisCall.getFollowupNotes()) ? "rowspan=\"2\"" : ""%> width="8" valign="top" nowrap>
    ...
    <dhv:evaluate if='<%= CallsListInfo.getExpandedSelection() && !"".equals(thisCall.getFollowupNotes()) %>'>
      <tr class="row<%= rowid %>">
